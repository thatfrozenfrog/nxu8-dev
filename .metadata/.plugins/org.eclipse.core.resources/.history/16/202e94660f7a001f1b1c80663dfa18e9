...
void set_bits(uint8_t *mem_address, int start_bit, int num_bits, uint8_t pattern, byte invert) {
    ...
    uint8_t start_mask;
    uint8_t end_mask;
    // Handle the case where all bits fit in one byte
    if (start_byte == end_byte) {
        // Create a mask for the bits within the byte
        mask = ((1 << num_bits) - 1) << (8 - start_offset - num_bits);
        if (invert) {
        	*(mem_address + start_byte) ^= mask;  // XOR để invert bit
        } else {
            *(mem_address + start_byte) = (*(mem_address + start_byte) & ~mask) | (pattern & mask);  // Set bits according to the pattern
        }
    } else {
        // Handle the first byte with a partial mask (from start_offset to the end of the byte)
        start_mask = 0xFF >> start_offset;
        if (invert) {
        	*(mem_address + start_byte) ^= start_mask;  // XOR để invert bit
        } else {
            *(mem_address + start_byte) = (*(mem_address + start_byte) & ~start_mask) | (pattern & start_mask);  // Set bits according to the pattern
        }

        // Handle full bytes between start and end (if any)
        for (i = start_byte + 1; i < end_byte; i++) {
            if (invert) {
            	*(mem_address + i) ^= 0xFF;  // XOR toàn bộ byte để invert
            } else {
                *(mem_address + i) = pattern;  // Set all bits to the pattern
            }
        }

        // Handle the last byte with a partial mask (from the start of the byte to end_offset)
        end_mask = ~(0xFF >> (end_offset + 1));
        if (invert) {
        	*(mem_address + end_byte) ^= end_mask;  // XOR để invert bit
        } else {
            *(mem_address + end_byte) = (*(mem_address + end_byte) & ~end_mask) | (pattern & end_mask);  // Set bits according to the pattern
        }
    }
}
