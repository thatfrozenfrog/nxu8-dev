...
1000 'If host does not have hackprog, then get one
hostprev$ = th_re$(way$,regpath$,i-2)
hostprev$ = th_re$(hostprev$,"\b[a-zA-Z0-9-]+\b") 'Weird fix for invisible added characters
? rev$ hostprev$ reset$

th_exec ("ftp " + hostprev$ )
th_exec ("get pcrack.exe"),out$
? out$
th_exec ("exit")
if pos(out$,"550 ") then ? hostprev$ " does not have pcrack!" : goto 1100
goto 51

1100 'Even previous host doesn't have pcrack, avoiding you all!
avoid$ = avoid$ + " " + th_re$( th_hostname$, "[^\s]+")
avoid$ = avoid$ + " " + hostprev$
th_exec ("exit")
th_exec ("exit")
goto 55